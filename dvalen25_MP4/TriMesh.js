   ...
   */
    ...
    */
    ...
    {    
        ...
        for (let i = 0; i < lines.length; i++) {
            let line = lines[i];
            let separated = line.split(/\b\s+(?!$)/); // by whitespace

            if (separated[0] == "v") {
                console.log("hello");
                // Push XYZ double
                this.vBuffer.push(parseFloat(separated[1]));
                this.vBuffer.push(parseFloat(separated[2]));
                this.vBuffer.push(parseFloat(separated[3]));
            } else if (separated[0] == "f") {
                // push XYZ int  -1 since they are 1 indexed in the .obj file
                this.fBuffer.push(parseInt(separated[1]) - 1);
                this.fBuffer.push(separated[2] - 1);
                this.fBuffer.push(separated[3] - 1);
    ...
    */    
    ...
    canonicalTransform(){
        //Your code here
        // Find longest side L
        let tVector = glMatrix.vec3.create();
        let L = 0;
        for (let i = 0; i < 3; i++) {
            // get biggest L but not on this assignment ;)
            let currLength = this.maxXYZ[i] - this.minXYZ[i];
            if (L < currLength) {
                L = currLength;
            }

            tVector[i] = (this.maxXYZ[i] + this.minXYZ[i]) * -0.5;

        }
        let scale = glMatrix.mat4.create();
        glMatrix.mat4.fromScaling(scale, glMatrix.vec3.fromValues(1.0/L, 1.0/L, 1.0/L));
        // Translate to move the box in the middle
        let translation = glMatrix.mat4.create();
        glMatrix.mat4.fromTranslation(translation, tVector);

        glMatrix.mat4.multiply(this.modelMatrix, scale, translation);
        // glMatrix.mat4.multiply(this.modelMatrix, translation, scale);
    }
